...
G_DEFINE_TYPE_WITH_PRIVATE(DiskeyInputListner,
                           diskey_input_listener,
                           G_TYPE_OBJECT)

static void
diskey_input_listener_class_init(DiskeyInputListnerClass* klass)
{
    GObjectClass* object_class = G_OBJECT_CLASS(klass);
}

static void
diskey_input_listener_keyboard_setup(DiskeyInputListner* input_listener)
{
    ...
    priv->keyboard_replay_xic = XCreateIC(priv->keyboard_replay_xim,
                                          XNClientWindow,
                                          priv->x11_replay_window,
                                          XNInputStyle,
...
static void
diskey_input_listener_keyboard_delete(DiskeyInputListner* input_listener)
{
...
gboolean
diskey_input_listener_keyboard_phantom_release(Display* display,
                                               XEvent* cur_event)
{
    diskey_debug(DEBUG_INPUT_LISTENER);
    XEvent* next_event;
...
void
diskey_input_listener_mouse_button_process(DiskeyInputListner* input_listener,
                                           XEvent* event)
{
    diskey_debug(DEBUG_INPUT_LISTENER);
...
static void
diskey_input_listener_keyboard_process(DiskeyInputListner* input_listener,
                                       XEvent* event)
{
    DiskeyInputListnerPrivate* priv;
    ...
    // Only handle keypress, then generate new key_data and append it to
    // `keyboard_data`
    if (event->type != KeyPress)
        return;

    KeyboardData* key_data = g_new(KeyboardData, 1);
    diskey_keyboard_event_data_generate(
      event, priv->last_keyboard_event, key_data);
    diskey_keyboard_event_data_modifier(
      priv->keyboard_replay_xic, event, key_data);
    key_data->key_symbol =
      XkbKeycodeToKeysym(event->xkey.display, event->xkey.keycode, 0, 0);
    if (key_data->key_symbol >= 32 && key_data->key_symbol <= 126) {
        key_data->symbol = g_strdup_printf("%c", key_data->key_symbol);
    } else {
        key_data->symbol = XKeysymToString(key_data->key_symbol);
    }

    g_print(
      "key_symbol=%d, symbol=%s\n", key_data->key_symbol, key_data->symbol);
...
void
diskey_input_listener_record_data_handler(DiskeyInputListner* input_listener,
                                          XRecordInterceptData* recorded_data)
{
    ...
    switch (wire_event->u.u.type) {
        case KeyPress:   // 2
        case KeyRelease: // 3
            x11_wireEvent_to_XKeyEvent(
              priv->x11_record_display, wire_event, &event);
            break;
        case ButtonPress:   // 4
        case ButtonRelease: // 5
            x11_wireEvent_to_XButtonEvent(
              priv->x11_record_display, wire_event, &event);
            break;
        case MotionNotify: // 6
            x11_wireEvent_to_XMotionEvent(
              priv->x11_record_display, wire_event, &event);
            break;
        ...
        case FocusIn:  // 9
        case FocusOut: // 10
            x11_wireEvent_to_XClientEvent(
              priv->x11_custom_atom, wire_event, &event);
            break;
        default:
            return;
    }
    XSendEvent(
      priv->x11_replay_display, priv->x11_replay_window, FALSE, 0, &event);
    diskey_debug(DEBUG_INPUT_LISTENER);
}

void
diskey_input_listener_record_context_callback(
  XPointer closure,
  XRecordInterceptData* recorded_data)
{
...
void
diskey_input_listener_record_context_setup(DiskeyInputListner* input_listener)
{
    /**
    ...
    record_context = XRecordCreateContext(
      priv->x11_control_display, 0, &clients, 1, &ranges, 1);
    XFree(ranges);
...
Window
diskey_input_listener_x11_replay_window_create(Display* display)
{
    diskey_debug(DEBUG_INPUT_LISTENER);
...
static void
diskey_input_listener_init(DiskeyInputListner* input_listener)
{
    DiskeyInputListnerPrivate* priv;
    ...
    priv->x11_replay_window =
      diskey_input_listener_x11_replay_window_create(priv->x11_replay_display);
...
DiskeyInputListner*
diskey_input_listener_new(DiskeyWindow* window)
{
...
void
diskey_input_listener_restart(DiskeyInputListner* input_listener)
{
    DiskeyInputListnerPrivate* priv;
...
void
diskey_input_listener_start(DiskeyInputListner* input_listener)
{
    DiskeyInputListnerPrivate* priv;
    ...
    Status status =
      XRecordEnableContextAsync(priv->x11_record_display,
                                priv->record_context,
                                diskey_input_listener_record_context_callback,
                                input_listener);
...
void
diskey_input_listener_stop(DiskeyInputListner* input_listener, GThread* thread)
{
    DiskeyInputListnerPrivate* priv;
...
void
diskey_input_listener_finalize(DiskeyInputListner* input_listener)
{
    DiskeyInputListnerPrivate* priv;
