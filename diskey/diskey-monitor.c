...
void diskey_monitor_on_size_changed_event(DiskeyScreen *screen,
                                       gpointer user_data) {
}

void diskey_monitor_on_monitors_changed_event(DiskeyScreen *screen,
                                           gpointer user_data) {}
...
DiskeyScreen *diskey_screen_get_default(DiskeyWindow *window) {
    ...
    */ 
    ...
    g_signal_connect(screen, "size-changed",
                     G_CALLBACK(diskey_monitor_on_size_changed_event), NULL);
    g_signal_connect(screen, "monitors-changed",
                     G_CALLBACK(diskey_monitor_on_monitors_changed_event), NULL);
    return screen;
}
...
gint diskey_monitor_check_id(gint monitor_id) {
    GdkDisplay *display = gdk_display_get_default();
