...
diskey_mouse_button_states_update(DiskeyMouse* mouse, MouseButtonData* data)
{
    ...
    state = &g_array_index(priv->button_states, MouseButtonData*, data->button);
    *state = data;
}
...
diskey_mouse_composite_pixbuf_by_button_states(DiskeyMouse* mouse)
{
    ...
    while (button_pixbufs) {
        ...
        gdk_pixbuf_composite(pixbuf_current,
                             priv->pixbuf,
                             0,
                             0,
                             width,
                                                   ...
                                                   int mouse_height)
{
    ...
    for (guint i = 0; i < priv->button_pixbufs_len; i++) {
        ...
        pixbuf = gdk_pixbuf_scale_simple(
          pixbuf_raw, dest_width, mouse_height, GDK_INTERP_BILINEAR);
