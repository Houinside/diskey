...
G_DEFINE_TYPE_WITH_PRIVATE (DiskeyWindow, diskey_window,
...
static gboolean
diskey_window_on_configure_event (GtkWidget *widget, GdkEvent *event,
                                  gpointer user_data)
{
...
static gboolean
diskey_window_on_draw_event (GtkWidget *widget, cairo_t *cr,
                             gpointer user_data)
{
...
gboolean
diskey_window_on_quit_event (DiskeyWindow *window, GdkEvent *event,
                             gpointer user_data)
{
  DiskeyWindowPrivate *priv;
  priv = diskey_window_get_instance_private (window);
...
diskey_window_init (DiskeyWindow *window)
{
  ...
  g_signal_connect (window, "configure-event",
                    G_CALLBACK (diskey_window_on_configure_event), NULL);
  g_signal_connect (window, "draw", G_CALLBACK (diskey_window_on_draw_event),
                    NULL);
  g_signal_connect (window, "delete-event",
                    G_CALLBACK (diskey_window_on_quit_event), NULL);

  DiskeyWindowPrivate *win_priv = diskey_window_get_instance_private (window);
  window->priv = win_priv;
  win_priv->timer_hide_event_source_id = 0; // g_timeout_add alwways return > 0
...
diskey_window_create (DiskeyApp *app)
{
  ...
  window = g_object_new (DISKEY_TYPE_WINDOW, "application", app, "type",
...
diskey_window_open (GApplication *app)
{
  ...
  win_priv->input_listener_thread
      = g_thread_new ("input listener thread", diskey_input_listener_start,
